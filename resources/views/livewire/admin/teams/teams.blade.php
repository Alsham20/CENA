                        ...
                        <table class="table table-centered w-100 dt-responsive nowrap" id="products-datatable">
                            ...
                            @foreach ($teams as $team)
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{$team?->media?->getUrlThumbnail() ?? asset('images/default-avatar.png')  }}" height="40" width="40" alt="table-user" class="me-2 rounded-circle">
                                    </td>
                                    ...
                                    <td>
                                        <span
                                            class="badge badge-outline-{{$team->is_private ? 'success' : 'warning'}}">{{$team->is_private ? 'Privé' : 'Public'}}</span>
                                    </td>
                                    ...
                                    <td class="table-action">
                                        @can('view teams')
                                            <a href="{{route('teams.show', $team->id)}}" > <i class="mdi mdi-eye text-info h3"></i></a>
                                        @endcan
                                        @can('edit teams')
                                                <a href="{{route('teams.edit', $team->id)}}" > <i class="mdi mdi-square-edit-outline text-primary h3"></i></a>
                                        @endcan
                                        @can('delete teams')
                                                <a href="javascript:void(0);" wire:click.prevent="$dispatch('confirm-delete',{{$team->id}})"> <i class="mdi mdi-delete text-danger h3"></i></a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <div class="row mt-3">
                        <div class="col-sm-12 col-md-5">
                            <div class="dataTables_info" id="products-datatable_info" role="status" aria-live="polite">
                                Afficher {{$teams->firstItem()}} a {{$teams->lastItem()}}
                                sur {{$teams->total()}}</div>
                        </div>
                            ...
                            <div class="dataTables_paginate paging_simple_numbers float-end"
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-end">
                                        <!-- Bouton Précédent -->
                                        <li class="page-item @if($teams->onFirstPage()) disabled @endif">
                                        ...
                                        @foreach ($teams->links()->elements[0] as $page => $url)
                                            <li class="page-item @if($teams->currentPage() == $page) active @endif">
                                                <a class="page-link" href="#" wire:click.prevent="gotoPage({{ $page }})">{{ $page }}</a>
                                            </li>
                                        @endforeach

                                        <!-- Bouton Suivant -->
                                        <li class="page-item @if(!$teams->hasMorePages()) disabled @endif">
                ...
                <div class="modal-body p-4">
                    <div class="text-center">
                        ...
                        @if($confirm_delete)
                            ...
                            <p class="mt-3">Voulez-vous supprimer ce membre? Cette action est irreversible</p>
