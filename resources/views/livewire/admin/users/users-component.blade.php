            ...
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        @can('create users')
                            <div class="col-sm-5">
                                <a href="{{route('users.create')}}" class="btn btn-success mb-2"><i
                                        class="mdi mdi-plus-circle me-2"></i> Ajouter un Utilisateur</a>
                            </div>
                        @endcan
                        <!-- end col-->
                    </div>
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-2">
                            <div class="mb-3">
                                <select wire:model.live="role" wire:change="resetPage" class="form-select"
                                        id="example-select">
                        ...
                        <table class="table table-centered w-100 dt-responsive nowrap" id="products-datatable">
                            ...
                            @foreach ($users as $item)
                                <tr>
                                    ...
                                    <td>
                                        @foreach ($item->sroles as $role)
                                            <span class="badge bg-info rounded-pill">{{$role->name}}</span>
                                        @endforeach
                                    </td>
                                    <td>
                                        <div class="form-check form-switch">
                                            <span class="ms-2">@if($item->is_suspended)
                                    ...
                                    <td>
                                        @can('view users')
                                            <a href="{{route('users.show',$item->id)}}" title="Details"> <i
                                                    class="mdi mdi-eye text-info h3"></i></a>
                                        @endcan
                                        @can('edit users')
                                            <a href="{{route('users.edit',$item->id)}}" title="Modifier"> <i
                                                    class="mdi mdi-square-edit-outline text-warning h3"></i></a>
                                        @endcan
                                        @can('delete users')
                                        ...
                                        @if($item->is_suspended)
                                            @can('unsuspend users')
                                                <a href="javascript:void(0);"
                                                   wire:click.prevent="unsuspend('{{$item->id}}')" title="Restaurer"> <i
                                                        class="mdi mdi-account-reactivate-outline text-success h3"></i></a>
                                            @endcan
                                        @else
                                            @can('suspend users')
                                                <a href="javascript:void(0);"
                                                   wire:click.prevent="suspend('{{$item->id}}')" title="Suspendre"> <i
                                                        class="mdi mdi-cancel text-danger h3"></i></a>
                                            @endcan
                                        @endif
                    ...
                    <div class="row">
                        <div class="col-sm-12 col-md-5">
                            <div class="dataTables_info" id="products-datatable_info" role="status" aria-live="polite">
                                Afficher {{$users->firstItem()}} a {{$users->lastItem()}} sur {{$users->total()}}</div>
                        </div>
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-end">
                                        ...
                                        @foreach ($users->links()->elements[0] as $page => $url)
                                            <li class="page-item @if($users->currentPage() == $page) active @endif">
                ...
                <div class="modal-body p-4">
                    <div class="text-center">
                        ...
                        @if($confirm_delete)
                            ...
                            <p class="mt-3">Vous etes sur de vouloir supprimer cet utilisateur. Cette action est
                                irreversible</p>
    ...
    Livewire.on('confirm-delete', id => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Livewire.dispatch('delete', {user_id: id});
                /*Swal.fire(
                    'Supprimé!',
