            ...
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        @can('create videos')
                        <div class="col-sm-5">
                            <a href="{{route('videos.create')}}" class="btn btn-success mb-2"><i
                                    class="mdi mdi-plus-circle me-2"></i> Ajouter une vidéo</a>
                        </div>
                        @endcan
                        <!-- end col-->
                    </div>
                        ...
                        <table class="table table-centered w-100 dt-responsive nowrap" id="products-datatable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge badge-outline-{{$video->is_published ? 'success' : 'warning'}}">{{$video->is_published ? 'Publié' : 'Non publié'}}</span>
                                    </td>
                                    <td style="width: 20%;white-space :normal">
                                        {{ ($video->date_video) ? \Carbon\Carbon::parse($video->date_video)->format('d/m/Y') : ""}}
                                    </td>
                                    ...
                                    <td class="table-action">
                                        ...
                                        @endcan
                                        @can('edit videos')
                                        <a href="{{route('videos.edit', $video->id)}}"> <i
                                                class="mdi mdi-square-edit-outline text-primary h3"></i></a>

                                        @endcan
                                        @can('delete videos')
                                        <a href="javascript:void(0);"
                                            wire:click.prevent="$dispatch('confirm-delete',{{$video->id}})"> <i
                                        ...
                                        @can('publish videos')
                                        <a href="javascript:void(0);"
                                            wire:click.prevent="publishVideo({{$video->id}})" title="Publié"> <i
                                                class="mdi mdi-bookmark-check text-success h3"></i></a>
                                        @endcan
                                        @else
                                        @can('unpublish videos')
                                        <a href="javascript:void(0);"
                                            wire:click.prevent="unpublishVideo({{$video->id}})"
                                            title="Dépublier"> <i
                    ...
                    <div class="row mt-3">
                        <div class="col-sm-12 col-md-5">
                            <div class="dataTables_info" id="products-datatable_info" role="status" aria-live="polite">
                                Afficher {{$videos->firstItem()}} a {{$videos->lastItem()}}
                                sur {{$videos->total()}}</div>
                        </div>
                            ...
                            <div class="dataTables_paginate paging_simple_numbers float-end"
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-end">
                                        ...
                                        <li class="page-item @if($videos->currentPage() == $page) active @endif">
                                        ...
                                        <!-- Bouton Suivant -->
                                        <li class="page-item @if(!$videos->hasMorePages()) disabled @endif">
