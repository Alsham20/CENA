...
<template>
  <div class="w-full h-full bg-white shadow-xl transition-all duration-300 flex flex-col justify-between">
    ...
    <div class="p-4 border-t-4 border-t-[#2D3748] border-b border-gray-200 flex items-center gap-4">
      ...
      <h3 class="text-[#2D3748] text-sm xl:text-xl font-bold">{{ document.name.slice(0, 50) }}
          {{ document.name.length > 50 ? '...' : '' }}</h3>
    </div>
    ...
    <div class="p-4 flex-1 flex flex-col justify-between">
      <div>
        <p class="text-[#2D3748] font-bold text-xs xl:text-lg mb-3">
          Date : {{ document.date_creation ? formatDate(document.date_creation) : '' }}
        </p>
        <p class="text-gray-700 text-sm xl:text-lg leading-relaxed">
          <span class="text-[#2D3748] font-bold">Objet : </span>
          {{ document.object.slice(0, 100) }}
          {{ document.object.length > 100 ? '...' : '' }}
        </p>
      </div>
    </div>
    ...
    <div class="flex px-4 pb-6 gap-3 text-xs xl:text-lg font-semibold mt-auto">
      <DocumentPreview :url="document.doc_path" type="application/pdf" />
      <!-- <button
      ...
      <button
        ...
        @click="handleDownload(document.doc_path, document.name + '.pdf')">
        Télécharger le PDF
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
...
const { isDownloading, error, downloadFile } = useFileDownload()
