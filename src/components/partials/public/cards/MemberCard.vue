...
const handleBiographyClick = () => {
  emit('viewBiography', props.member)
}

const handleSocialClick = (url: string, platform: string) => {
  emit('openSocialLink', url, platform)
}
</script>
...
<template>
  <div data-aos="zoom-in" class="relative bg-white rounded-lg shadow-lg overflow-visible pt-20 pb-6 px-6 border-b-4 border-[#11845a] flex flex-col justify-between h-full">
    ...
    <!-- Contenu de la carte -->
    <div class="text-center pt-16 flex-1 flex flex-col justify-between">
      <div>
        <!-- Nom et prénom -->
      ...
      <div class="flex items-center justify-between mt-auto">
        <!-- Bouton Biographie -->
        <button 
          @click="handleBiographyClick"
          class="bg-[#0E6258] text-white px-4 py-2 rounded-md text-sm xl:text-lg font-medium hover:bg-[#11845a] transition-colors"
        >
          BIOGRAPHIE
        </button>
